...
export function CreateGoal() {
  ...
  async function handleCreateGoal(data : CreateGoalForm){
    await createGoal({title : data.title,desiredWeekFrequency:data.desiredWeekFrequency})

    queryClent.invalidateQueries({queryKey: ['summary']})
    queryClent.invalidateQueries({queryKey: ['pending-goals']})
        ...
        <form onSubmit={handleSubmit(handleCreateGoal)} className="flex-1 flex flex-col justify-between ">
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              ...
              <Input id="title" autoFocus placeholder="Praticar exercicios" {...register("title")} />
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="title">Quantas vezes na semana?</Label>
                <Controller control={control} name='desiredWeekFrequency' defaultValue={3} render={({field }) => {
                  return (
